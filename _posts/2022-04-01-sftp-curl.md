...
Did you know you can use curl as a minimal SFTP client? In this post we will guide you through the various curl commands for [uploading](#upload-a-file-via-curl), [downloading](#download-a-file-via-curl), and [listing](#list-files-via-curl) files on a SFTP server, with ready to use snippets you can copy/paste in your terminal.
...
A first technique is the default private key authentication which uses your private key available from `~/.ssh/id_rsa`:
